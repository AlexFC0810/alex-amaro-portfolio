...
const files=['index.html','creative.html','assets/portfolio.css','assets/portfolio.js','assets/og-creative-first.png'];
...
while(Date.now()-started<300000){
 try{
  ...
  const samples=['claims.json','Alex-Amaro-Resume.pdf',items[0].thumb,...items.filter(x=>x.file.startsWith('assets/portfolio-static/')).map(x=>x.file)];
  ...
  console.log(JSON.stringify({result:'PASS',verifiedAt:new Date().toISOString(),url:base,commit:process.env.GITHUB_SHA||null,staticCreatives:items.length,checked,phoneCallPlaced:false,crmChanged:false},null,2));process.exit(0);
