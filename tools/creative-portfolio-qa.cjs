...
const packetIndex=[];
async function packet(name,file,width=720){const buf=await sharp(file).resize({width,withoutEnlargement:true}).webp({quality:38,effort:6}).toBuffer();const b64=buf.toString('base64'),parts=[];for(let i=0;i<b64.length;i+=6000){const f=`_review/packs/${name}-${String(i/6000+1).padStart(2,'0')}.txt`;fs.writeFileSync(f,b64.slice(i,i+6000));parts.push(f);}packetIndex.push({name,width,bytes:buf.length,parts});}
(async()=>{
 ...
 try{
  ...
  for(const [w,h] of [[375,812],[760,900],[1440,900]]){
   ...
   if(w===375){await page.locator('.lightbox-media').dispatchEvent('touchstart',{changedTouches:[{clientX:270,clientY:180}]});await page.locator('.lightbox-media').dispatchEvent('touchend',{changedTouches:[{clientX:80,clientY:185}]});check((await page.locator('#lightbox-count').innerText())!==initial,'lightbox swipe');}
   ...
   if(w===375){await page.locator('.menu-toggle').click();check(await page.locator('#nav-links').isVisible(),'mobile nav');await page.locator('#nav-links a[href="#voice"]').click();check(await page.locator('.menu-toggle').getAttribute('aria-expanded')==='false','nav closes after link');}
